...
import { db } from './firebaseConfig'; // Ensure you have your Firebase config
import AsyncStorage from '@react-native-async-storage/async-storage';

const RejectedAudits = ({ route, navigation }) => {
  const [selectedReason, setSelectedReason] = useState('');
  const reasons = [
    'Not enough time',
    'Already handling too many audits',
  ...
  const { auditId } = route.params; // Receive auditId via route params
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              // Remove the audit from AcceptedAudits subcollection
              const acceptedAuditDocRef = doc(
                db,
                'Profile',
                userId,
                'AcceptedAudits',
              ...
              await deleteDoc(acceptedAuditDocRef);
              ...
              Alert.alert('Error', 'Failed to remove the audit. Please try again.');
            }
          },
        },
      ]
